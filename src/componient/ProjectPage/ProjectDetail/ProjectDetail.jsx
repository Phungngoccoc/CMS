...
import { Subscribe, fetchDataDetailProject } from '../../../services/userServices';
import { fetchDataUser } from '../../../services/userServices';
const ProjectDetailPage = React.memo(() => {
    ...
    useEffect(() => {
        const fetch = async () => {
            try {
                const res = await fetchDataDetailProject(id);
                if (res.status < 400) {
                    if (res?.data?.data?.status === STATUS.PUBLISH) {
                        const data = res.data.data;
                        setUserCreated(data.user_created ? data.user_created : null);
                        const dataPush = {};
                        setDate(res.data.data.date_created);
                        setImagePage(data?.image);
                        data.translations?.map((lang) => {
                            if (Object.values(LANGUAGES).includes(lang.languages_code)) {
                                dataPush[lang.languages_code] = lang;
                            }
                        });
    ...
    return (
        <>
            <Helmet>
                <title>{dataPage?.en?.content_main}</title>
                <meta name="description" content={dataPage?.en?.content} />

                {/* open graph */}
                <meta property="og:title" content={dataPage?.en?.content_main} />
                <meta property="og:description" content={dataPage?.en?.content} />
                <meta property="og:image" content={`${import.meta.env.VITE_URL_BACKEND}/assets/${imagePage}`} />
                <meta property="og:url" content={`${import.meta.env.VITE_URL_FRONTEND}/projects/${id}`} />
                                ...
                                <div className="child px-4 ">
                                    <div className="date d-flex align-item-center justify-content-end py-4">
                                        <span>
                                            ...
                                            {new Date(date ? date : Date.now()).toLocaleDateString('vi-VN')}
                                        </span>
                                    </div>
                                    <div className="py-4 content">
                                        <h1 style={{ color: `${companyInfor?.data[0]?.color_text_heading}` }}>
                                            {dataPage[languageApp]['content_main']}
                                        </h1>
                                        ...
                                        {userInfo?.data && (
                                            <div className="social-icon d-flex py-4 ps-3">
                                                ...
                                                {userInfo?.data?.link_linkedin && (
                                                    <div className="item">
                                                        <a href={userInfo?.data?.link_linkedin} target="_blank">
            ...
            {isShowRelated && <RelatedProject idSkip={id} />}
        </>
    );
});
